'use strict';

import Color from 'color';
import Picker from './Picker';

/**
 * @class HSLPicker
 */
export default class HSLPicker extends Picker {
	/**
	 * @returns {number}
	 * @public
	 */
	get saturation() {
		return this._saturation;
	}

	/**
	 * @param {number} value
	 * @public
	 */
	set saturation(value) {
		this._saturation = Number(value);
		this._render();
	}

	/**
	 * @param {number} width
	 * @param {number} height
	 * @param {Color|string|object} [color]
	 * @param {number} saturation
	 * @public
	 */
	constructor(width, height, color, saturation = 100) {
		super(width, height, color);

		this._saturation = Number(saturation);

		this._render();
	}

	/**
	 * @param {Event} event
	 * @private
	 */
	_change(event) {
		if (this.selecting) {
			this._render();

			let x = event.pageX - this.element.offsetLeft,
				y = event.pageY - this.element.offsetTop,
				data = this.context.getImageData(x, y, 1, 1).data;

			this._color = Color({
				'r': data[0],
				'g': data[1],
				'b': data[2]
			});

			this.notify(HSLPicker.CHANGE, {'color': this._color});

			this._drawSelection(x, y);
		}
	}

	/**
	 * @param {number} x
	 * @param {number} y
	 * @private
	 */
	_drawSelection(x, y) {
		this.context.beginPath();
		this.context.arc(x, y, 6, 0, 2 * Math.PI, false);
		this.context.lineWidth = 1;
		this.context.strokeStyle = this._color.luminosity() > 0.5 ? '#000' : '#fff';
		this.context.stroke();
	}

	/**
	 * @private
	 */
	_render() {
		if (!isNaN(this._saturation)) {
			let tempCanvas = document.createElement('canvas'),
				numColors = 360,
				tempContext,
				i;

			tempCanvas.setAttribute('width', numColors.toString());
			tempCanvas.setAttribute('height', numColors.toString());

			tempContext = tempCanvas.getContext('2d');

			for (i = 0; i < numColors; i++) {
				let linearGradient = tempContext.createLinearGradient(0, 0, 1, numColors);

				linearGradient.addColorStop(0, 'hsl(' + i + ', ' + this._saturation + '%, 100%)');
				linearGradient.addColorStop(0.5, 'hsl(' + i + ', ' + this._saturation + '%, 50%)');
				linearGradient.addColorStop(1, 'hsl(' + i + ', ' + this._saturation + '%, 0%)');

				tempContext.fillStyle = linearGradient;
				tempContext.fillRect(i, 0, 1, numColors);
			}

			this.context.drawImage(tempCanvas, 0, 0, this.element.width, this.element.height);
		}
	}
}